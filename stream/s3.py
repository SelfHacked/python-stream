...
class S3File(object):
    ...
    def get_body(self) -> _StreamingBody:
    ...
    def as_iter(self, *, lines=False) -> _typing.Iterator[bytes]:
        body = self.get_body()
        try:
            if lines:
    ...
    def as_stream(self, *, lines=False) -> _Stream[bytes]:
        return _IterStream(self.as_iter(lines=lines))
    ...
    def copy_to(
        ...
        if in_memory:
            other.upload(_BytesIO(self.read_all()))
        else:
            with _TemporaryFile() as f:
